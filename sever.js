 ...
 */
const BASE_URLS = [
  'http://207.154.244.233:1000/api/v2/hianime',
  'http://164.92.202.165:2000/api/v2/hianime'
];
...
app.get("/search", async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(
      `http://207.154.244.233:1000/api/v2/hianime/search`,
      {
...
app.get("/subepisodes", async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(
      `http://207.154.244.233:1000/api/v2/hianime/anime/${animeId}/episodes`
    );
...
app.get("/fetchsubtitle", async (req, res) => {
  ...
  try {
    const servers = ["hd-1", "hd-2", "hd-3"]; // List of possible servers to try
    let category = "sub"; // Default category
    let response = null;

    for (const server of servers) {
      try {
        // Attempt to fetch subtitles for the current server and category
        response = await axios.get(
          "https://aniwatch-api-production-528d.up.railway.app/api/v2/hianime/episode/sources",
          { params: { animeEpisodeId, server, category } }
        );

        // If the response is successful and contains data, break out of the loop
        if (response.data.success && response.data.data?.tracks?.length) {
          break;
        }
      } catch (error) {
        // Log the error and continue to the next server
        console.warn(`Failed to fetch from server '${server}' with category '${category}':`, error.message);
      }
    }

    // If no successful response, try the 'raw' category
    if (!response || !response.data.success || !response.data.data?.tracks?.length) {
      category = "raw";
      for (const server of servers) {
        try {
          response = await axios.get(
            "http://207.154.244.233:1000/api/v2/hianime/episode/sources",
            { params: { animeEpisodeId, server, category } }
          );
    ...
    // If no valid response is obtained after trying all servers and categories
    if (!response || !response.data.success) {
      return res.status(500).json({
        success: false,
        error: "Failed to fetch subtitle sources after trying all servers and categories.",
      });
    }
...
app.post('/translateArabic', async (req, res) => {
  ...
  try {
    ...
    // Initialize GoogleGenerativeAI with the translator's API key
    const translatorApiKey = translator.geminiApiKey;
    const genAIForTranslator = new GoogleGenerativeAI(translatorApiKey);
    const modelForTranslator = genAIForTranslator.getGenerativeModel({
    ...
    // Split cues into chunks (5 cues each by default)
    const chunkedCues = chunkArray(cues, 5);
    const translatedCuesParts = [];

    // Translate each chunk
    for (let chunkIndex = 0; chunkIndex < chunkedCues.length; chunkIndex++) {
      const cueChunk = chunkedCues[chunkIndex];
      const combinedText = cueChunk
        .map((c, idx) => `Cue ${chunkIndex * 5 + idx + 1}:\n${c.text}`)
        .join('\n---\n');

      // Translate the combined text
      const translatedChunkText = await translateText(combinedText, modelForTranslator);

      // Split the translation back into individual cues
      const splitTranslatedTexts = translatedChunkText
        .split('\n---\n')
        .map(text => {
...
/**
 * Parse VTT text into an array of cues with { timing, text }.
 */
function parseVTT(vttText) {
  ...
  while (i < lines.length) {
    ...
    cues.push({ timing: timingLine, text: textLines.join('\n') });
  }
  return cues;
}
 ...
 */
async function translateText(text, translatorModel) {
  const prompt = `Translate the following cues to Arabic. Maintain the numbering and separators exactly as provided, so the cues align properly. Only translate the text content.\n\n${text}`;
  try {
    const result = await translatorModel.generateContent(prompt);
...
/**
 * Reconstruct VTT from an array of cues.
 */
...
/**
 * Extract username from a given URL path.
 */
...
app.post('/translatore', async (req, res) => {
  const { name, geminiApiKey, githubApiKey } = req.body;

  if (!name || !geminiApiKey) {
    return res.status(400).json({
      success: false,
      message: 'Name and Gemini API Key are required.',
    });
  }

  try {
    // Check if translator with the same name already exists
    const existingTranslator = await Translator.findOne({ name: name.trim() });

    if (existingTranslator) {
      return res.status(409).json({
        success: false,
        message: 'A translator with this name already exists.',
      });
    }
    ...
    return res.status(201).json({
      success: true,
      message: 'Translator saved successfully.',
      data: newTranslator,
    });
  } catch (error) {
    console.error("Error in /translatore:", error.message || error);
    return res.status(500).json({
      success: false,
      message: 'Internal server error while saving translator.',
    });
  }
});
...
app.get('/translator', async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, data: translators });
  } catch (error) {
    console.error('Error in GET /translator:', error.message);
    return res.status(500).json({ success: false, message: 'Server error' });
  }
});
